...
class LLAMA(nn.Module):
    def __init__(self,in_dim,in_frame,hidden_dim,n_heads,out_dim,num_ik=4,num_layer=2,device='cuda'):
        super(LLAMA,self).__init__()
        self.freqs_cis=precompute_freqs_cis(hidden_dim*n_heads,in_dim).to(device)
        ...
        self.ik=RealNVP(num_ik,out_dim,out_dim,device=device)
    def forward(self,x):
        self.ik.to(x.device)
